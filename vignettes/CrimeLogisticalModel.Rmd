...
author: "Anthony A, Ivan Tikhonov, Seung min song, Alice A. Friedman"
...
all_train_raw <- read_csv("https://raw.githubusercontent.com/IvanGrozny88/DATA-621-HW3/main/crime-training-data_modified.csv")
all_test <- read_csv("https://raw.githubusercontent.com/IvanGrozny88/DATA-621-HW3/main/crime-evaluation-data_modified.csv")
```
...
Noticeably, some features strongly show a difference between the two plots, such as `indus` whereas others are more similar.
We should expect all of the models to weigh `indus` heavily.
...
The variable `cols_to_bin` contains the names of the columns in the dataset that we want to bin, and breaks contains the breaks for the buckets we want to use for each variable.
...
Notably, the variables `nox` and `rad` both have very high significance below 0.001 so we will definitely use these in the future.
This is expected based on their distributions with respect to the target variable, above.
...
Before we begin stepwise addition and subtraction, let's also run a model with only variables with significance below 0.1 value.
...
Here, most of the features stayed the same as before, except nox scaled up much lower along with the intercept.
...
```{r}
modelThree <- MASS::stepAIC(modelOne, direction = "backward", trace = FALSE)
summary(modelThree)
```

## Model 4 - Forward Stepwise from No Features
...
Therefore, let's instead run it from a no feature model.

```{r}
modelFour <- stats::glm(target ~ nox, data = train_clean, family = 'binomial') %>%
  MASS::stepAIC(., direction = "forward", trace = FALSE)
summary(modelFour)
```
...
To compare logistical models, we can assess accuracy, precision, deviance, AIC, and so on.
It's not clear whether we should prefer precision or specificity here so we will use the derived F1 metric.
So first, we will do chi square testing to prove the statistical validity of the models and then look at F1 to replace old models.
...
TO DO Don't we want to use the information from the plots to inform our choice of model? Or some logical intepretation of the data?
