...
rename_crime <- function(df){
  df <- df %>% 
    dplyr::rename(
      ...
      `Weighted Mean Distance to Boston Employment Centers` = dis,
      ...
      `Med. Val. of Owner-Occupied Homes ($1ks)` = medv, 
...
```{r}
summary(rename_crime(all_train))
```
...
ggplot(reshape2::melt(rename_crime(all_train_raw), id=c("Borders Charles", "High Crime Status")), aes(x = value)) + 
    geom_density(fill = "skyblue") + 
    facet_wrap(~variable, scales = 'free', labeller=label_wrap_gen(width = 15, multi_line = TRUE)) 
```

For categorical variable `chas` (which measures proximity to the Charles River), we can look at a confusion matrix table to make sure that we have enough observations for all levels:

```{r}
confusionMatrix = xtabs(~ target + chas, data=all_train_raw)
confusionMatrix
```
