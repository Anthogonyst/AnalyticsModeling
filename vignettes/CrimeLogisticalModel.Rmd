...
author: "Anthony A, Ivan Tikhonov, Seung min song, Alice A. Friedman, Charles Ugiabe"
...
all_train_raw <- read.csv("https://raw.githubusercontent.com/IvanGrozny88/DATA-621-HW3/main/crime-training-data_modified.csv")
...
#make predictions
probabilities1 <- modelOne %>% predict(validation, type = "response")
...
probabilities2 <- modelTwo %>% predict(validation, type = "response")
...
```{r}
modelThree <- glm(target ~., data = train_clean, family = "binomial") %>% MASS::stepAIC(direction = "backward", trace = FALSE)

summary(modelThree)

probabilities3 <- modelThree %>% predict(validation, type = "response")
...
probabilities4 <- modelFour %>% predict(validation, type = "response")
...
```{r}
ValidationPipeline(modelTwo, validation, "target", "Model 1")
```
