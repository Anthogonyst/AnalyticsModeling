...
Next, let's examine the distribution of the features with respect to the target variable. Shown below, in light blow are the distribution of the variable where crime is high (`target` is 1), and in dark blue are the distribution where crime is low. 

Noticeably, some features strongly show a difference between the two plots, such as `indus` whereas others are more similar. We should expect all of the models to weigh `indus` heavily. Additionally, we can expect other features wiht notable differences to get weighed with some significance such as `nox` and `lstat` and `rad`. Features like `chas`, where the distributions are nearly identical, are not likely targets for the model. This makes sense, as there is no obvious causal reason why bordering the Charles River should affect crime.
...
Notably, the variables `nox` and `rad` both have very high significance bellow 0.001 so we will definitely use these in the future. This is expected based on their distributions with respect to the target variable, above.
