...
```{r}
glimpse(rename_crime(all_train))
```
...
```{r}
cor_matrix <- cor(rename_crime(all_train))
cor_with_target <- cor_matrix["High Crime Status", colnames(cor_matrix) != "High Crime Status"]
cor_with_target
```
...
```{r}
datasub <-  reshape2::melt(rename_crime(all_train), id=c("Borders Charles", "High Crime Status"))
...
confusionMatrix = xtabs(~ target + chas, data=all_train)
confusionMatrix
```
