...
```{r plot_all, warning=FALSE message=FALSE}
...
ggplot(cleanData, aes(x=INCOME)) +
  geom_freqpoly(aes(color=JOB))
...
# See common-helpers.R
RemoveCols(cleanData, factorVars) %>% #remove categorical or binary variables
  .[, sapply(., is.numeric)] %>%
    stats::cor(., use = "na.or.complete") %>%
      # See common-stats.R
      PlotCorrEllipse(.) 

# Numeric corrlational matrix for continuous variables
...
binomialDF <- cleanData %>% mutate(
  SEX = SEX %>% plyr::mapvalues(from = c("M", "F"), to = c(1, 0))
) %>% select

# Define a function to test correlation between binomial features and other features
categorical_corr_test <- function(df, cols) {
  ...
  for (col in cols){
    ...
    for (j in 1:(ncol(df))) {
	    s <- summary (glm(catVar ~ df[, j], family = binomial))
...
categorical_corr_test(binomialDF, binomials)
```
We can also visualize each variable in comparison to the target.
...
#drop AGE nas
df <- trainData %>% tidyr::drop_na(AGE)
...
All the remaining features with missing values have correlation with at least one other feature, except **CAR_AGE**. **CAR_AGE** can be said to plausibly be missing completely at random, and so those rows can also be dropped without any expectation of affecting the model performane.

```{r drop_missing_car_age}
trainData <- df %>% tidyr::drop_na(AGE, CAR_AGE)
```
