...
#so is this no longer raw then? we shouldn't reuse the term in that case
# AA: i agree, but i didnt want to break someone else's downstream analysis; see impute steps below
...
## Missing values
The rawData data frame contains `r sum(colSums(is.na(rawData)))`` missing values, representing `r sum(!complete.cases(rawData))` out of `r nrow(rawData)` observations. Most observations wiht
