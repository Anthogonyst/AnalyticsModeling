...
MAR_test <- function(df, cols) {
  ...
  for (col in cols){
    ...
    print(paste("Feature with missing values", col))
...
Missing values for **CAR_AGE**, **HOME_VAL**, **BLUEBOOK**, and **JOB** have no correlation with other features. These variables can therefore be said to plausibly be missing completely at random, and so those rows can also be dropped without any expectation of affecting the model performance. Only **YOJ** and **INCOME** have correlations and so should therefore have imputed values. 

```{r drop_missing_car_age}
trainData <- cleanData 
trainData["JOB"][cleanData["JOB"]==""] <- NA 
imputeData <- trainData %>%  tidyr::drop_na(AGE, CAR_AGE, JOB, BLUEBOOK, HOME_VAL) 
```
...
imputeData <- imputeData %>% dplyr::select(-INDEX) 
...
We will create several model iterations to create two predictions -- a model that predicts the continuous variable **TARGET_AMT** and one that predicts the binary target,  **TARGET_FLAG**.

## Predicting **TARGET_AMT**

Stepwise regression analysis was performed. The purpose of stepwise regression is to identify the subset of predictors that best explain the response variable. In this case, the response variables are KIDSDRIV, AGE, HOMEKIDS, YOJ, INCOME, HOME_VAL, TRAVTIME, BLUEBOOK, TIF, OLDCLAIM, CLM_FREQ, MVR_PTS, and CAR_AGE. The model selects the best subset of these predictors by iteratively adding or removing variables based on statistical criteria, in this case the Akaike Information Criterion (AIC).

The final model contains the variables KIDSDRIV, HOME_VAL, BLUEBOOK, TIF, CLM_FREQ ,MVR_PTS, CAR_AGE. This subset of variables yielded the lowest AIC value among all possible subsets of variables considered by the algorithm. 
...
set.seed(333)
trainIndex <- sample(1:nrow(imputeData), round(0.7*nrow(imputeData)), replace = FALSE)
trainData <- imputeData[trainIndex, ] %>% dplyr::select(-TARGET_FLAG)
testData <- imputeData[-trainIndex, ] %>% dplyr::select(-TARGET_FLAG)

model_step <- stepAIC(lm(TARGET_AMT ~ ., data = trainData))
```
```{r stepAIC_res}
summary(model_step)

predictions <- predict(model_step, testData)

#plot(model_step)
cor(predictions, testData$TARGET_AMT)
```

# Data Transformations

**EDUCATION** is an ordinal variable which maps in the real word quite closely to years. We will transform this ordinal variable into a numeric variable.
```{r edu}
imputeDataT <- imputeData %>%
  mutate(
    EDUCATION = EDUCATION %>% plyr::mapvalues(from=ed_levels, to=edYears) %>% as.numeric()
  )
```

```{r stepAIC_transformed, results='hide'}
library(MASS)

set.seed(333)
trainIndex <- sample(1:nrow(imputeDataT), round(0.7*nrow(imputeDataT)), replace = FALSE)
trainDataT <- imputeDataT[trainIndex, ] %>% dplyr::select(-TARGET_FLAG)
testDataT <- imputeDataT[-trainIndex, ] %>% dplyr::select(-TARGET_FLAG)

model_stepT <- stepAIC(lm(TARGET_AMT ~ ., data = trainDataT))
```
```{r stepAIC_T_Res}
summary(model_stepT)

predictionsT <- predict(model_stepT, testDataT)

#plot(model_step)
cor(predictionsT, testDataT$TARGET_AMT)
```

Interestingly, this transformation does not significantly alter the results!

Next, we can attempt to increase the predictive value of the model by reducing correlated features and dropping features with low p values.
```{r linear2}
set.seed(333)
trainIndex <- sample(1:nrow(imputeDataT), round(0.7*nrow(imputeData)), replace = FALSE)
trainData2 <- imputeData[trainIndex, ] %>% dplyr::select(-TARGET_FLAG)
testData2 <- imputeData[-trainIndex, ] %>% dplyr::select(-TARGET_FLAG)

model_linear <- lm(TARGET_AMT ~ KIDSDRIV + HOME_VAL + TIF + OLDCLAIM + CLM_FREQ + CAR_AGE , data = trainData)

summary(model_linear)

predictions_linear <- predict(model_linear, testData2)

#plot(model_step)
cor(predictions_linear, testDataT$TARGET_AMT)

qplot(predictions_linear, testData2$TARGET_AMT) + lab("Predicted vs. Actual Results")
```
