...
The final model contains the variables YOJ, HOMEKIDS, TRAVTIME, OLDCLAIM, CAR_AGE, AGE, KIDSDRIVE, TIF, BLUEBOOK, HOME_VAL, MVR_PTS, CLM_FREQ, TARGET_AMT.
This subset of variables yielded the lowest AIC value among all possible subsets of variables considered by the algorithm. 
...
# Seung-min
The ultimate goal of the analysis is to predict the probability of a car insurance claim being filed (TARGET_FLAG) based on the variables presented. The models developed are tools for achieving this goal by learning patterns in the data that can be used to make predictions about new cases.

Using the summary output of a linear regression model in R, the TARGET_FLAG and predicted_flag were compared, resulting in a matching rate of well above 80%.

```{r echo=FALSE}
TARGET_AMT <- 4.655e-05
KIDSDRIV <- 0.04989
AGE <- -1.864e-03
HOMEKIDS <- 8.459e-03
YOJ <- -1.877e-03
...
testData <- read.csv("C:/Users/SeungminSong/Downloads/621R/insurance_training_data.csv")
predicted <- predict(lm(TARGET_FLAG ~ TARGET_AMT + KIDSDRIV + AGE + HOMEKIDS + YOJ + HOME_VAL + TRAVTIME + BLUEBOOK + TIF + OLDCLAIM + CLM_FREQ + MVR_PTS + CAR_AGE, data = testData), testData)

testData <- na.omit(testData)

testData$predicted_flag <- ifelse(predict(model, testData, type="response") > 0.5, 1, 0)

head(testData[, c("TARGET_FLAG", "predicted_flag")], n=10)
```

```{r}
testData$matching_rate <- ifelse(testData$TARGET_FLAG == testData$predicted_flag, 1, 0)
matching_rate <- sum(testData$matching_rate) / nrow(testData) * 100
cat("Matching rate:", matching_rate, "%")
```
